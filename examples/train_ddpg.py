...
def setup_exps_rllib(flow_params,
    ...
    bets_params_path = "/home/edoardo_reply/ray_results/DDPG/DDPG_CustomNormalizedMultiAgentAccelPOEnv-v1_dc581331_2025-02-22_19-29-37o8amt_hl/params.json"

    with open(bets_params_path, "r") as f:
        best_params = json.load(f)

    # 🔹 Creazione della configurazione basata sui migliori parametri trovati
    config.update({
        "num_workers": best_params.get("num_workers", 1),  # Se non c'è, default a 1
        "num_gpus": best_params.get("num_gpus", 0),
        "gamma": best_params.get("gamma", 0.99),
        "tau": best_params.get("tau", 0.01),
        "horizon": best_params.get("horizon", 1000),
        "actor_hiddens": best_params.get("actor_hiddens", [128, 128]),
        "critic_hiddens": best_params.get("critic_hiddens", [128, 128]),
        "actor_lr": best_params.get("actor_lr", 1e-4),
        "critic_lr": best_params.get("critic_lr", 1e-3),
        "train_batch_size": best_params.get("train_batch_size", 1024),
        "buffer_size": best_params.get("buffer_size", 100000),
        "exploration_noise_type": best_params.get("exploration_noise_type", "ou"),
        "exploration_ou_theta": best_params.get("exploration_ou_theta", 0.15),
        "exploration_ou_sigma": best_params.get("exploration_ou_sigma", 0.2),
        "exploration_ou_noise_scale": best_params.get("exploration_ou_noise_scale", 0.1),
    })
