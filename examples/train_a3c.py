...
def setup_exps_rllib(flow_params,
    ...
    """Configurazione dell'esperimento RLlib per A3C."""
    ...
    # 🔄 Modifica algoritmo da PPO a A3C
    alg_run = "A3C"
    ...
    config.update({
        "num_workers": n_cpus,  # Più worker per A3C asincrono
        "sample_batch_size": 10,  # Dimensione dei batch di rollout
        "train_batch_size": horizon * n_rollouts,  # Dimensione del batch di addestramento
        "gamma": 0.99,  # Discount rate
        "lambda": 1.0,  # GAE(lambda)
        "grad_clip": 40.0,  # Clipping dei gradienti per stabilità
        "lr": 0.0001,  # Learning rate
        "lr_schedule": None,  # Nessuna schedulazione LR
        "vf_loss_coeff": 0.5,  # Coefficiente della loss della funzione di valore
        "entropy_coeff": 0.01,  # Coefficiente di entropia per esplorazione
        "min_iter_time_s": 5,  # Tempo minimo per ogni iterazione
        "sample_async": True,  # 🔥 A3C funziona in modo asincrono
...
def train_rllib(submodule, flags):
    """Esegue il training utilizzando A3C in RLlib."""
    import ray
