...
from flow.core.rewards import desired_velocity, penalize_standstill, rl_forward_progress, throughput_reward
...
class CustomNormalizedMultiAgentAccelPOEnv(MultiEnv):
    ...
    * target_velocity: desired velocity for all vehicles in the network, in m/s
    ...
    Rewards:
        ...
        4. A penalty for lane changes.
        5. A penalty for small time headways among autonomous vehicles.
        6. A reward for throughput.
...
class VariantNormalizedMultiAgentAccelPOEnv(MultiEnv):
    """Custom multi-agent acceleration environment with combined rewards.
    ...
    * target_velocity: desired velocity for all vehicles in the network, in m/s
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        # Ricompensa basata sulla velocità desiderata
        desired_vel_reward = desired_velocity(self, fail=kwargs['fail'])

        # Penalizzazione veicoli fermi
        standstill_penalty = penalize_standstill(self, gain=0.5)  # Ridotto l'impatto

        # Ricompensa per il progresso in avanti (aumentato il guadagno)
        forward_progress_reward = rl_forward_progress(self, gain=0.5)  # Prima era 0.1

        # Penalizzazione per cambio corsia e ritardi (ridotti)
        penalize_lane_change = punish_rl_lane_changes(self) * 0.5  # Ridotta la penalità
        minimum_delay = min_delay(self) * 0.5  # Penalità ridotta

        # **Nuova ricompensa basata sul throughput (aumentata)**
        throughput_bonus = throughput_reward(self, gain=0.5)  # Prima era 0.1

        # **Combinare tutte le metriche con pesi aggiornati**
        combined_reward = (
            desired_vel_reward +
            standstill_penalty +
            forward_progress_reward +
            minimum_delay +
            penalize_lane_change +
            throughput_bonus
        )

        cost1 = rewards.desired_velocity(self, fail=kwargs["fail"])

        # Penalizzazione per headway troppo piccolo
        cost2 = 0
        t_min = 1  # Tempo minimo tra due veicoli
        for rl_id in self.k.vehicle.get_rl_ids():
            lead_id = self.k.vehicle.get_leader(rl_id)
        ...
        # Dare maggiore peso ai nuovi incentivi
        total_reward = (combined_reward * 0.2) + cost1 + (cost2 * 0.5)
